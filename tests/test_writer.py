...
from borehole_app.models import Borehole, MainFileData, BasicLayer, TestRecord, ProjectData
...
class TestRenderFunctions:
    ...
    def test_test_file_lines(self) -> None:
        """测试生成试验文件行。"""
        borehole = Borehole(prefix="ZK1", folder=Path("/data"), hole_type="ZK")
        borehole.tests["o"] = [
        ...
        result = test_file_lines(borehole, "o")
        
        assert len(result) == 2
        assert result[0] == "1,5,S1"
        assert result[1] == "6,10,S2"
    
    def test_test_file_lines_with_empty_values(self) -> None:
        """测试生成试验文件行（包含空值）。"""
        borehole = Borehole(prefix="ZK1", folder=Path("/data"), hole_type="ZK")
        borehole.tests["o"] = [
            TestRecord(values=["1", "5", "S1", ""]),
...
class TestGenerateBorehole:
    ...
    def test_generate_borehole_with_old_prefix(self, tmp_path: Path) -> None:
        ...
        generated = generate_borehole(borehole, old_prefix="ZK1")
