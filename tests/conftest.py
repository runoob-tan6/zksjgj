...
def sample_project_folder(tmp_path: Path, sample_borehole_data: dict[str, str]) -> Path:
    ...
    for suffix, content in sample_borehole_data.items():
        file_path = borehole_folder / "ZK1" if suffix == "main" else borehole_folder / f"ZK1.-{suffix}"
...
def mock_borehole():
    ...
    borehole = Borehole(prefix="ZK_TEST", folder=Path("/tmp/test"), hole_type="ZK")
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "ui: marks tests that require UI (deselect with '-m \"not ui\"')")
